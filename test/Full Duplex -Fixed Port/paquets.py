...
def crc_valide(paquet: bytes):
    """Vérifie si le CRC d'un paquet est valide"""
    pack = paquet[:-4]
    crc = paquet[-4:]
    return crc == binascii.crc32(pack).to_bytes(4,'big')

def octets_valide(octets: bytes):
    """Vérifie si le CRC d'une séquence d'octets est valide"""
    if len(octets) >6: #Les ACKS et les NACKS n'ont pas de CRC
        return crc_valide(octets)
    return True
