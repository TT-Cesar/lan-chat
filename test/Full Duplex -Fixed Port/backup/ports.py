...
# print(adresses_multicast)
# print(type(adresses_multicast))
...
class Utilisateur:
    def __init__(self, noms: List[str], prenoms: List[str], cle_publique: Optional[bytes]):
        self.noms = noms
...
class Session:
    """
    Pour créer un flux d'octets entre appareils
    """ 
    def __init__(self,sock_reception: socket.socket,  destinataire: Appareil, fdc: Optional[callable], cle: Optional[bytes]):
...
class Chats:
    """
    Pour créer et maintenir des sessions entre plusieurs appareils.
    
    """
    contenu_chaines = [None for i in range(301)]
    sock_de_recherche = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # Socket pour l'écoute des chaines multicast

    def __init__(self,ip: str, sessions: List[Session]=[], etats_chaines: List[bytes] = []):
        self.ip = ip
        self.sessions = sessions
        self.chaine_multicast = None #La chaine multicast de cet appareil.
        ...
        for port in ports_decoutes:
            try:
                ...
                if port == ports_decoutes[-1]:
                    del self # On ne peux pas commencer un chat si aucun port n'est dispo
        ...
        for adresse in adresses_multicast:
            ...
            Chats.sock_de_recherche.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, mreq)
    ...
    def publier_message(message: bytes):
        """
        Publie un méssage sur sa chaine multicast.
        """


    def broadcast(groupe: socket.socket):
        """
        Pour broadcast en continu. Doit être appelé dans un thread

        Structure d'un message de chaine multicast :
        [Nom(s): 200 octets] [Prénom(s): 200 octets] [taille de la clé publique: 2 octets] [Clé publique: 1024 octets] [infos sup: 40 octets] [CRC: 4octets] Total: 1470 octets

        """
        
        
    def actualiser_contenu_chaines():
        """
        ...
        Structure d'un message de chaine multicast :
        [Nom(s): 200 octets] [Prénom(s): 200 octets] [taille de la clé publique: 2 octets] [Clé publique: 1024 octets] 
        [infos sup: 40 octets] [CRC: 4 octets] Total: 1470 octets

        Chaque chaine attendra 100ms apres chaque envoi de son statut

        Pour se connecter où lancer une discussion, Alice savoir l'états des 300 chaines.
        Pour vérifier le statut, Alice suit chaque chaine en regardant les états des chaines
        dans son tableau. 

        Donc ici le tableau doit être mis à jour en continu dans le tableau dans un thread qui s'executera indéfiniment.
